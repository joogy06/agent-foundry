...
- [x] **Roundtable skill initiative → shipped as `avengers` design + build** (2026-07-11/12): full forge COMPLEX cycle run (4 approaches + UX + codex/agy diverge, dual challengers, spec review); design approved — `docs/plans/2026-07-11-avengers-skill-design.md`; ADRs 0001/0002 in the new agent-foundry wiki; WP-1 vertical slice + live value-proof session passed the user gate.
- [x] **avengers build COMPLETE** (2026-07-12, run `wf_69c15d0a-7d4`, 7/7 stages): 59 files, 123/123 tests (independently re-run by lead), G_CLASSIFY --verify-diff + G_SECURITY clean. All five WPs + finalize delivered per `docs/plans/2026-07-11-avengers-skill-design.md`; wiki-v02 carries the component page + sprint-note.
- [ ] **avengers post-build follow-ups (USER actions)**: (1) repo→live byte-sync of `skills/avengers/` (25 files) + 3 upstream edits (`adversarial-team-brainstorm/references/arbiter-synthesis.md`, `forge/SKILL.md`, `visual-companion/SKILL.md`) — fold into the standing deferred byte-sync, then re-run identity_check; (2) `~/.codex/skills/avengers` symlink; (3) commit the working tree (~59 files uncommitted on `feat/codex-effort-policy` — consider a dedicated branch/commit split: effort-policy vs avengers); (4) first REAL avengers convene on a fresh decision to start the §12 success-criteria clock (memory-compounding test by session ≥3); (5) v1.1 backlog per design §14 (global memory tier, vindicated/refuted calibration, mid-phase interjections, repo-local overrides).
...
- [x] Decide `~/.codex/config.toml` default — LOWERED `max` → `xhigh` (2026-07-11, user-applied after the avengers WP-1 live deliberation: converged 2-1, unanimous CHANGE_NEEDED on the incumbent; 3 trip-wires recorded in `.avengers/sessions/20260711-2003-codex-effort-default/outcome/decision-record.md`; per-call pins remain mandatory).
