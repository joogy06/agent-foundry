...
Reference file for the `modern-frontend` skill. Covers testing (Jest/Testing Library, Angular testing, Playwright E2E, MSW), build tooling (Vite, Nx monorepo, GitHub Actions CI/CD, environment variables), and accessibility (WCAG 2.1 semantic HTML, keyboard navigation, color contrast, accessibility checklist).
...
Vite 8 (stable 2026-03) ships **Rolldown** as its single default bundler.
Rollup's object-form `build.rollupOptions.output.manualChunks = { vendor: [...] }`
is **not supported** under Rolldown and will fail the build; the function-form
`manualChunks` and `advancedChunks` are deprecated compatibility paths. Manual
vendor splitting now uses `build.rolldownOptions.output.codeSplitting.groups`
(entries of `{ name, test: /regex/, priority? }`).

```typescript
// vite.config.ts  (Vite 8.x, Rolldown default)
...
export default defineConfig(({ mode }) => ({
  ...
  build: {
    ...
    // Rolldown chunk splitting — replaces Rollup's object-form manualChunks.
    rolldownOptions: {
      output: {
        codeSplitting: {
          groups: [
            { name: 'vendor', test: /node_modules[\\/](?:react|react-dom|react-router-dom)[\\/]/, priority: 100 },
            { name: 'query', test: /node_modules[\\/]@tanstack[\\/]/ },
            { name: 'ui', test: /node_modules[\\/](?:@headlessui|@heroicons)[\\/]/ },
          ],
        },
      },
    },
