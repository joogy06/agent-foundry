#!/usr/bin/env python3
"""test_plugin_express.py — fixture-driven smoke test for the express extractor."""
...
FIXTURE = _SKILL / "fixtures" / "express-minimal"
EXPECTED = json.loads((FIXTURE / "expected-edges.json").read_text())["expected"]


def _run_plugin():
    plugins = discover_plugins()
    assert "express" in plugins, f"express plugin missing; found={list(plugins)}"
    ep = plugins["express"]
    resolver = ComponentResolver(FIXTURE / "progress" / "contract-map.yaml", FIXTURE)
    sources = sorted((FIXTURE / "src").rglob("*.js"))
    edges = list(ep.extract_edges(
        project_dir=FIXTURE,
        symbols={"by_file": {}, "by_name": {}},
        source_files=sources,
        workspace_tree_hash="0" * 40,
        extractor_version=ep.version,
        config={},
...
def test_express_expected_subset():
    actual = _run_plugin()
    def key(e):
...
def test_express_edges_validate():
    actual = _run_plugin()
...
def test_express_deterministic():
    a = _run_plugin()
    b = _run_plugin()
...
def main():
    tests = [test_express_expected_subset, test_express_edges_validate, test_express_deterministic]
    failed = []
    for t in tests:
        try:
            t()
