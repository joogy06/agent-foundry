...
When authoring or reviewing a skill that must work across multiple AI CLIs (Claude Code, Gemini CLI, Codex CLI, GitHub Copilot CLI), follow these rules. **Read this file first** before drafting or modifying any cross-tool skill.
...
The four major AI coding CLIs in 2026 (Claude Code, Gemini CLI, Codex CLI, GitHub Copilot CLI) all support skill-like extensions, but with subtle differences. A skill that works perfectly on Claude may silently fail on Gemini if the frontmatter has extra fields. A reference file that loads on Gemini may exceed Copilot's instruction-file budget.

The rules here are the **intersection** — strict enough that any compliant skill works on all four tools.

## The five hard rules (memorise these)

1. **Frontmatter is `name + description` only.** No `allowed-tools`, no `model`, no `tools`, no `context`, no anything else. Gemini silently rejects extras; Claude tolerates them; Copilot ignores them; Codex flags them. Strict intersection wins.

2. **Naming**: `^[a-z0-9-]+$`, max 64 chars. No underscores, no uppercase, no dots. Same constraint in all four tools.

3. **Body**: SKILL.md `<500` lines. Move detail to `references/*.md`. Same hard limit in Gemini, recommended in Claude/Codex.
...
| [`install-matrix.md`](install-matrix.md) | Setting up symlinks across `~/.claude/`, `~/.gemini/`, `~/.codex/` |
| [`agents-md-canonical.md`](agents-md-canonical.md) | Designing AGENTS.md / CLAUDE.md / GEMINI.md content |
| [`hooks-portability.md`](hooks-portability.md) | Authoring hooks that must work in both Claude and Gemini |
| [`headless-invocation.md`](headless-invocation.md) | Translating `claude -p` patterns to `gemini -p` and `copilot -p` |
...
skills/    skills/    (no skills)
   ...
   `gemini skills link <path>` for Gemini
...
| Don't | Why |
|---|---|
| Add `allowed-tools` to frontmatter for Claude convenience | Gemini silently rejects extras → skill never loads on Gemini |
| Use uppercase or underscores in `name` | All four tools require lowercase + hyphens |
| Inline >500 lines in SKILL.md | Gemini's hard limit; Claude's recommended limit |
| Reference skills by absolute path | Path differs across machines; use `<skill-name>` and let the agent resolve |
| Share a single hooks file across Claude and Gemini | Schemas differ subtly. Edit Claude source, run `gemini hooks migrate`, commit both. |
