...
`index.md` is a shared mutable file — multiple sessions can race on it. Apply the **hash-verified write protocol** (SKILL.md HARD-RULE on shared mutable files):

```bash
# 1. Snapshot
EXPECTED=$(sha256sum index.md | cut -d' ' -f1)
EXPECTED_LINES=$(wc -l < index.md)

# 2. Backup
cp -p index.md "index.md.bak.$(date -u +%Y%m%dT%H%M%SZ)"

# 3. Compose edits in memory (Read tool / planning) — do NOT write yet

# 4. Re-check hash immediately before write
CURRENT=$(sha256sum index.md | cut -d' ' -f1)
if [ "$CURRENT" != "$EXPECTED" ]; then
  echo "DRIFT DETECTED — abort. expected=$EXPECTED current=$CURRENT" >&2
  exit 11
fi

# 5. Apply edits atomically (Edit tool) — DO NOT plain `>` redirect

# 6. Post-write: log new hash + line count
NEW_HASH=$(sha256sum index.md | cut -d' ' -f1)
NEW_LINES=$(wc -l < index.md)
echo "index.md updated: $EXPECTED -> $NEW_HASH; lines $EXPECTED_LINES -> $NEW_LINES" >> _maintenance/index-edits.log
```

Append new page entries to the relevant category section:
...
**Same protocol applies to** `_maintenance/link-index.md` (regenerated by lint --fix), `WIKI.md` (rare schema bumps), and `log.md` (per-batch entries — append-only but still racy). Per-page files under `wiki/<category>/<slug>.md` are page-private and don't need the hash check unless they're being mass-rewritten.

**Backup retention**: the timestamped `.bak.YYYYMMDDTHHMMSSZ` files accumulate; they're safe to gitignore but should NOT be auto-deleted — they're the forensics trail when a drift abort surfaces. Recommend a 30-day sweep policy in `_maintenance/`.
